...
import { CreateWalletRequest, CreateWalletResponse } from '../../controllers/dtos';
...
describe('WalletController', () => {
      ...
      providers: [
        {
          ...
          useValue: {
            create: jest.fn().mockResolvedValue({ id: 1, description: 'Entrada', value: -100, paymentMethod: 'PIX' } as unknown as CreateWalletResponse),
            findAll: jest.fn().mockResolvedValue({ wallets: [{ id: 1, description: 'Entrada', value: -100, paymentMethod: 'PIX' }] } as unknown as AllWallet),
            findOne: jest.fn().mockResolvedValue({ id: 1, description: 'Entrada', value: -100, paymentMethod: 'PIX' } as unknown as CreateWalletResponse),
            update: jest.fn().mockResolvedValue({ id: 1, description: 'Entrada', value: -100, paymentMethod: 'PIX' } as unknown as CreateWalletResponse),
  ...
  describe('create', () => {
    it('should create a new wallet', async () => {
      const walletDto: CreateWalletRequest = { description: 'Entrada', value: -100, paymentMethod: 'PIX' };
      const req = { user: { sub: 1 } };
      const result = await walletController.create(walletDto, req);
      expect(result).toEqual({ id: 1, description: 'Entrada', value: -100, paymentMethod: 'PIX' });
  ...
  describe('findAll', () => {
    it('should return all wallets', async () => {
      ...
      expect(result).toEqual({ wallets: [{ id: 1, description: 'Entrada', value: -100, paymentMethod: 'PIX' }] });
